...
import com.ikkong.core.jfinal.ext.plugin.quartz.Quartz;
import com.ikkong.core.jfinal.ext.plugin.quartz.QuartzPlugin;
...
public class MainConfig extends JFinalConfig implements Const{
	...
	public void configPlugin(Plugins me) {
		// 配置插件
		me.add(new EhCachePlugin());//可以提高系统的并发访问速度
	    ...
	    me.add(new ShiroPlugin(Routes.getRoutesList().get(0)));
	    
	    Quartz q = new Quartz();
        q.setJobsFile("quartz/jobs.properties");
...
//		String path = getProperty("image.headPath");
		
		ServletContext sc = JFinal.me().getServletContext();
		String sys = System.getProperty("os.name");//Linux
		String head = "Linux".equals(sys) ? "/" : "";
		String path = head + sc.getRealPath("/").replaceFirst("/", "");

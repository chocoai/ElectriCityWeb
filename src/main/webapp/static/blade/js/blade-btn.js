...
(function () {
    ...
    tmsp_btn.prototype.itemClick = function () {
        ...
        if (this.alias == "authority") {
            ...
            $.post(ctx + "/role/getPowerById", { id: ids }, function (data) {
                if (data.code === 0) {
                    ...
                    _this.open(_this.url + split + ids + urlParaSeparator + roleName);
                }
                else{
        ...
        if (this.alias == "assign") {
        	...
        	this.open(this.url + split + ids + urlParaSeparator + name + urlParaSeparator + roleID);
            return;
        }
        if (this.alias == "agent") {
            if (rows == 0) {
            ...
            var Name=rowData.NAME;
        	this.open(this.url + split + ids + urlParaSeparator + Name,"","1");
            return;
        }
        ...
        // 电工用户解除锁定
        if (this.alias == "relieve") {
        	if (rows == 0) {
                layer_alert('请选择一条数据!', "warn");
                return;
            }
        	if(rowData.statusName=='正常'){
        		layer_alert('正常状态操作无效!', "warn");
        		return;
        	}
            var _this = this;
            var url= _this.url;
            layer.confirm('是否确定解除挂起或冻结？', {
                icon: 3,
                btn: ['确定', '取消'] //按钮
            }, function () {
                $.post(url, { ids: ids }, function (data) {
        ...
        if (this.alias == "dgUser_hangup") {
        	if (rows == 0) {
                layer_alert('请选择一条数据!', "warn");
                return;
            }
        	if(rowData.statusName=='挂起'){
        		layer_alert('已是挂起状态,操作无效!', "warn");
        		return;
        	}
            var _this = this;
            var url= _this.url;
            layer.confirm('是否确定挂起该电工用户？', {
                icon: 3,
                btn: ['确定', '取消'] //按钮
            }, function () {
                $.post(url, { ids: ids }, function (data) {
        ...
        if (this.alias == "dgUser_hangup" 
        	|| this.alias == "dgUser_frozen" 
        		|| this.alias == "dgUser_disable") {
        	if (rows == 0) {
                layer_alert('请选择一条数据!', "warn");
                return;
            }
        	...
        	if(this.alias == "dgUser_hangup"){
        		sname = "挂起";
        	} else if(this.alias == "dgUser_frozen") {
        		sname = "冻结";
    		} else if(this.alias == "dgUser_disable") {
    			sname = "注销";
			}
        	
        	var tips = "是否确定"+sname+"该电工用户？";
        	if(rowData.statusName==sname){
        		layer_alert('已是['+sname+']状态,操作无效!', "warn");
        		return;
        	}
            var _this = this;
            var url= _this.url;
        ...
        if (this.alias == "auditCheck") {
        	if (rows == 0) {
                layer_alert('请至少选择一条数据!', "warn");
                return;
            }
        	var rowDatas = getRowsData();
        	for (var i = 0; i < rowDatas.length; i++) {
        		if(rowDatas[i].statusFlag=='审核通过'||rowDatas[i].statusFlag=='审核失败'){
            		layer_alert('选择项中有已审核记录,所选项必须都为未审核状态!', "warn");
            		return;
            	}
        	}
        ...
        if($('#grid-table-2').length > 0){
        	//jqGrid2 
            var tab2Ids = getGrid2Xls().join(",");
            var tab2Rows = getGrid2Xls().length;
            var tab2RowData = getGrid2RowData();
            
            if (this.alias == "workOrderAudit") {
            	if (tab2Rows == 0) {
                    layer_alert('请选择一条数据!', "warn");
                    return;
                }
            	if(tab2RowData.statusName != '发布中'){
            		layer_alert('[发布中]的工种单才能审核!', "warn");
            		return;
            	}
            	var _this = this;
                var url= _this.url;
                layer.confirm('是否确定审核通过, 并开始接受招标？', {
                    icon: 3,
                    btn: ['审核通过', '取消'], //按钮
                    ...
                    $.post(url, { ids: tab2Ids}, function (data) {
                        if (data.code === 0) {
                            layer_alert(data.message, "success");
                            reloadGrid2();
                            _this.initOther();
            ...
            if (this.alias == "workOrderProgress") {
            	if (tab2Rows == 0) {
                    layer_alert('请选择一条数据!', "warn");
                    return;
                }
            	this.open(this.url + split + tab2Ids);
                return;
            }
            
            if (this.alias == "workOrderApply") {
            	if (tab2Rows == 0) {
                    layer_alert('请选择一条数据!', "warn");
                    return;
                }

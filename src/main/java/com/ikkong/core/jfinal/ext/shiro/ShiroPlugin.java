...
public class ShiroPlugin implements IPlugin {
	...
	private static final Class<? extends Annotation>[] AUTHZ_ANNOTATION_CLASSES = new Class[] {
			...
			RequiresGuest.class, RequiresAuthentication.class ,RequiresUrlPermission.class};
	...
	private AuthzHandler createAuthzHandler(
		...
		// 至少有一个注解
		List<AuthzHandler> authzHandlers = new ArrayList<AuthzHandler>(6);
	...
	private void scanAnnotation(List<AuthzHandler> authzArray,
		...
		for (Annotation a : annotations) {
			...
			} else if (a instanceof RequiresUrlPermission){
				authzArray.set(5, new UrlPermissionAuthzHandler(a));
			}
		}
	}

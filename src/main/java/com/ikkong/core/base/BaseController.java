...
public class BaseController extends Controller implements Const, ConstShiro, ConstCurd, ConstCache {
	 ...
	 * @return
	 */
	protected DgUserToken dgUserToken() {
		String headToken = getPara(CommConts.TOKEN_KEY);
		if (StringUtils.isNotBlank(headToken)) {
			return CacheKit.get(ConstCache.CLIENT_CACHE, headToken);
		}
		return null;
	}

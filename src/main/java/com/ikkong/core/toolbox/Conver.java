...
import com.ikkong.core.exception.ToolBoxException;
import com.ikkong.core.jfinal.ext.kit.JStrKit;
import com.ikkong.core.toolbox.kit.DateTimeKit;
import com.ikkong.core.toolbox.support.BasicType;
...
public class Conver {
	...
	public static Object parseBasic(Class<?> clazz, String valueStr) {
		...
		switch (basicType) {
			...
			case SHORT:
				...
				return StringUtils.isBlank(valueStr) ? 0 : Short.valueOf(valueStr);
			case INT:
				return StringUtils.isBlank(valueStr) ? 0 : Integer.parseInt(valueStr);
			case INTEGER:
				return StringUtils.isBlank(valueStr) ? 0 : Integer.valueOf(valueStr);
			case LONG:
				if(clazz == long.class) {
					return Long.parseLong(valueStr);
				}
				return StringUtils.isBlank(valueStr) ? 0 : Long.valueOf(valueStr);
			case DOUBLE:
				if(clazz == double.class) {
					return Double.parseDouble(valueStr);
				}
				return StringUtils.isBlank(valueStr) ? 0.0 : Double.valueOf(valueStr);
			case FLOAT:
				if(clazz == float.class) {
					return Float.parseFloat(valueStr);
				}
				return StringUtils.isBlank(valueStr) ? 0.0 : Float.valueOf(valueStr);
			case BOOLEAN:
	...
	public static <T> Integer[] toIntArray(boolean isIgnoreConvertError, T... values) {
		...
		for(int i = 0; i < values.length; i++) {
			...
			if(null == v && isIgnoreConvertError == false) {
				throw new ToolBoxException(JStrKit.format("Convert [{}] to Integer error!", values[i]));
			}
			ints[i] = v;
		}
		return ints;
	...
	public static <T> Long[] toLongArray(boolean isIgnoreConvertError, T... values) {
		...
		for(int i = 0; i < values.length; i++) {
			...
			if(null == v && isIgnoreConvertError == false) {
				throw new ToolBoxException(JStrKit.format("Convert [{}] to Long error!", values[i]));
			}
			longs[i] = v;
		}
		return longs;
	...
	public static <T> Double[] toDoubleArray(boolean isIgnoreConvertError, T... values) {
		...
		for(int i = 0; i < values.length; i++) {
			...
			if(null == v && isIgnoreConvertError == false) {
				throw new ToolBoxException(JStrKit.format("Convert [{}] to Double error!", values[i]));
			}
			doubles[i] = v;
		}
		return doubles;
	...
	public static <T> Float[] toFloatArray(boolean isIgnoreConvertError, T... values) {
		...
		for(int i = 0; i < values.length; i++) {
			...
			if(null == v && isIgnoreConvertError == false) {
				throw new ToolBoxException(JStrKit.format("Convert [{}] to Float error!", values[i]));
			}
			floats[i] = v;
		}
		return floats;
	...
	public static <T> Boolean[] toBooleanArray(boolean isIgnoreConvertError, T... values) {
		...
		for(int i = 0; i < values.length; i++) {
			...
			if(null == v && isIgnoreConvertError == false) {
				throw new ToolBoxException(JStrKit.format("Convert [{}] to Boolean error!", values[i]));
			}
			bools[i] = v;
		}
		return bools;
	...
	public static String convertCharset(String str, String sourceCharset, String destCharset) {
		if(JStrKit.hasBlank(str, sourceCharset, destCharset)) {
			return str;
		}

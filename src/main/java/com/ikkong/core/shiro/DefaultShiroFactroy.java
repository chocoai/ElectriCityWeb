...
public class DefaultShiroFactroy implements IShiro{
	...
	public User user(String account) throws AuthenticationException{
		User user = Blade.create(User.class).findFirstBy("account = #{account}", Record.create().set("account", account));
		// 账号不存在
		if (null == user) {
			throw new AuthenticationException(account+"账号不存在");
		}
		// 账号未审核
		if (user.getStatus() == 3 || user.getStatus() == 4) {
			throw new AuthenticationException(account+"账号未审核");
		}
		// 账号被冻结
		if (user.getStatus() == 2 || user.getStatus() == 5) {
			throw new AuthenticationException(account+"账号被冻结");
		}
		return user;
	}
	...
	//----------------------------------------------------------------------
	public DgUser dgUser(String account) throws AuthenticationException {
		DgUser user = Blade.create(DgUser.class).findFirstBy("phoneno = #{account}", Record.create().set("account", account));
		// 账号不存在
		if (null == user) {
			throw new AuthenticationException(account+"账号不存在!");
		}
		// 账号被注销不能登录
		if (user.getStatus() == UserStatus.DISABLE.getStatusVal()) {
			throw new AuthenticationException(account+"账号已被注销!");
		}
		return user;
	}

	public ShiroDgUser shiroDgUser(DgUser user) {
		return new ShiroDgUser(user.getId(), user.getPhoneno(), user.getName());
	}

	public SimpleAuthenticationInfo dgUserInfo(ShiroDgUser shiroUser, DgUser user, String realmName) {
		String credentials = user.getUserpwd();
		// 密码加盐处理
		String source = user.getSalt();
		ByteSource credentialsSalt = new Md5Hash(source);
		return new SimpleAuthenticationInfo(shiroUser, credentials, credentialsSalt, realmName);

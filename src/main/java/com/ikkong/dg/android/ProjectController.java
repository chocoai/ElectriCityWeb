...
public class ProjectController extends BaseController {
	...
	public void add() {
		...
		Integer overflag = getParaToInt("overflag");
	...
	private WorkOrder buildItem(Items vo, Long dgUserId, Integer proId) {
		...
		//如果是发布状态更新发布时间 modify by xqliu
		if (vo.getStatus() == ItemStatus.PUBLISH.getStatusVal()) {
	...
	public void query() {
		...
		if (status != null && status > -1) {
			paraMap.set("status", status);//modify by xqliu
			sqlBuf.append(" and a.overflag = #{status}");
		}
	...
	public void modify() {
		...
		try {
			if (projServ.update(proj)) {
				...
				if (array != null && !array.isEmpty()) {
					Long dgUserId = dgUserToken().getUserId();
					updateItems(array, pid, dgUserId);
				}
				String removes = getPara("remove");
				if (StringUtils.isNotBlank(removes)) {
					workOrderServ.deleteByIds(removes.trim());
				}
	...
	private void updateItems(JSONArray array, Long pid, Long dgUserId) {
		for (Object json : array) {
			WorkOrder item = null;
			Items vo = JSON.parseObject(json.toString(), Items.class);
			if (vo.getId() < 1) {
				item = new WorkOrder();
				item.setOrder_id(pid);
				item.setCreate_time(new Date());
				item.setCreate_id(dgUserId);
				item.setVersion(1);
			} else {
				item = workOrderServ.findById(vo.getId());
			...
			if (item.getId() != null && item.getId() > 0) {
				if (!workOrderServ.update(item)) {
					renderJson(error("子项目记录更新失败" + item.getId()));
					return;
				}
			} else {
				try {
					workOrderServ.save(item);
				} catch (NullPointerException e) {
					e.printStackTrace();
					renderJson(error("服务端异常，数据库为空约束字段值未设置！"));
					return;
				}
			}
		}

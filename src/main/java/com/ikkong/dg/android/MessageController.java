...
public class MessageController extends BaseController {
	...
	public void query() {
		...
		try {
			...
			if (resultPage != null && !resultPage.getRows().isEmpty()) {
				updateMsgStatus(resultPage.getRows());
	...
	private void updateMsgStatus(List<Object> listMsg) {
		List<Messages> updateList = new ArrayList<Messages>();
		for(Object obj : listMsg) {
			Map<String, Object> map = (Map<String, Object>) obj;
			if(map.get("status").equals(MsgStatus.UNREAD.getStatusVal())) {
				Messages msg = msgServ.findById(map.get("id"));
				msg.setStatus(MsgStatus.READED.getStatusVal());
				msg.setRead_time(new Date());
				msg.setVersion(msg.getVersion()+1);
				updateList.add(msg);
		...
		if(!updateList.isEmpty()) {
			msgServ.updateBathById(updateList);
		}
	}
}
